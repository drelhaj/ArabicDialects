...
public class ARFFCreator {
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException {
		...
		String datasetDirectory = "dataset2";//where your dataset is (should contain sub-directories for each class (label or attribute).
		...
		createArffHeader(arffDirectory+"/AllClassesTruncate15.arff", Arrays.toString(classes).trim(),"text String");//method to create the ARFF file, the third argument is the datatype you may need to change this manually at some point (e.g. "String, String, Int, Int")

		for (int x = 0; x < classes.length; x++) {
			System.out.println(classes[x]);
			String[] classData = readFiles(datasetDirectory, classes[x]);
			classData = clean(classData);
